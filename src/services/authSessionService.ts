  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (transaction.set) {
      ...
      const secPayload: any = {
        ...
        isActive: true,
        lastActive: serverTimestamp(),
        createdAt: isSecDoc && secSnap.data()?.createdAt ? secSnap.data().createdAt : serverTimestamp()
      };
      transaction.set(securitySessionRef, secPayload, { merge: true });
    }
  });
