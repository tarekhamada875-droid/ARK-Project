...
const replacements = {
  ...
  '<yg ': '<Sliders ',
  '<xu ': '<AlertTriangle ',
  '<Fs ': '<XIcon ', // Avoid 'X' conflicting with variable X
  '<Xa ': '<Users ',
  '<cE ': '<Menu ',
  '<El ': '<Users ',
  '<Ha ': '<Search ',
...
import { 
  LogOut, Car, XCircle, Menu, X as XIcon, Users, Zap, ChevronDown, 
  Bell, PieChart, Sliders, AlertTriangle, Gift, Clock, Search
} from 'lucide-react';
...
const pt = (ts: any) => ts && ts.toDate ? ts.toDate() : new Date(ts);
const Ac = (g: any) => g.balance || 0; // rough approximation for Ke
const Qt = { currentUser: null, onAuthStateChanged: (cb: any) => { cb(null); return () => {} } }; // mock auth
const me = { subscribeToGarageRechargeLogs: () => () => {}, subscribeToSubscribers: () => () => {} }; // mock db
const Rn = { play: () => {} };
...
// Fix closing
code = code.replace(/KO = memo\(\(\{(.*?)\}\)/s, 'memo((props: any) => {\nconst { $1 } = props;');
